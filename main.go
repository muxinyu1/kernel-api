...
func restart(c *gin.Context) {
	cmd := exec.Command("kernel", "restart")
	err := cmd.Run()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status":  "error",
...
func status(c *gin.Context) {
	cmd := exec.Command("kernel", "status")
...
func setDomain(c *gin.Context) {
	...
	cmd := exec.Command("kernel", "--set-domain", req.Domain)
	err := cmd.Run()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status":  "error",
...
func setIp(c *gin.Context) {
	...
	cmd := exec.Command("kernel", "--set-ip", req.IPPlan)
	err := cmd.Run()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status":  "error",
